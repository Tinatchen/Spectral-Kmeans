...
def calc_affinity(Mat,neark,sigma):
    Msize = Mat.shape[0]
    ...
    for i in range(Msize):
        ...
        for j in range(neark):                                          #k-nearst,k=8
            idxj = ptseucli[j,1]
            affMat[i,idxj] = affPoint(Mat[i, :], Mat[idxj, :], 1/(2*sigma*sigma))  # choose sigma=1
        affMat[i,i]=0
    return  affMat
...
def spectrum_clustring(Mat,k,sigma):
    n=Mat.shape[0]
    affMat = calc_affinity(Mat,k,sigma)
...
nearkMAT = np.arange(1, 199, 2)
sigamaMAT = np.arange(0.1, 19.9, 0.2)
for i in range(99):
    # sigma=2
    # k = nearkMAT[i]
    k=8
    sigma=sigamaMAT[i]
    Accu=spectrum_clustring(dataset,k,sigma)
    plt.scatter(k,Accu)
plt.show()
